...
const Home = () => {
  ...
  const categories = [
    {
      ...
      description: language === 'es'
        ? 'Noticias oficiales y anuncios sobre River Plate y el foro'
        : 'Official news and announcements about River Plate and the forum',
      threadCount: 5,
      postCount: 23
    },
    {
      ...
      description: language === 'es'
        ? 'Discusiones generales sobre River Plate'
        : 'General discussions about River Plate',
      threadCount: 12,
      postCount: 87
    },
    {
      ...
      description: language === 'es'
        ? 'Discusiones sobre partidos pasados y futuros, y eventos relacionados con River Plate'
        : 'Discussions about past and upcoming matches, and events related to River Plate',
      threadCount: 8,
      postCount: 64
    }
  ];
  ...
  const recentThreads = [
    ...
    },
    {
      id: 3,
      title: language === 'es'
        ? 'Rumores de fichajes para la próxima temporada'
        : 'Transfer rumors for next season',
            ...
            {categories.map(category => (
              <div key={category.id} className="category-card">
                ...
                <div className="category-stats">
                  <span>{language === 'es' ? 'Temas: ' : 'Threads: '}{category.threadCount}</span>
                  <span>{language === 'es' ? 'Mensajes: ' : 'Posts: '}{category.postCount}</span>
                </div>
              </div>
            ))}
          </div>
            ...
            {recentThreads.map(thread => (
              <div key={thread.id} className="thread-card">
                ...
                <div className="thread-meta">
                  <span>{language === 'es' ? 'Por: ' : 'By: '}{thread.author}</span>
                  <span>{language === 'es' ? 'Categoría: ' : 'Category: '}{thread.category}</span>
                </div>
                <div className="thread-stats">
                  <span>{language === 'es' ? 'Respuestas: ' : 'Replies: '}{thread.replies}</span>
                  <span>{language === 'es' ? 'Vistas: ' : 'Views: '}{thread.views}</span>
                  <span>{language === 'es' ? 'Último mensaje: ' : 'Last post: '}{formatDate(thread.lastPost)}</span>
                </div>
              </div>
            ))}
          </div>
